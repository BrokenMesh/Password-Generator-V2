import { CRC32, Random } from "/js/bm-randomart/bm-random.js"
import { renderGLSL } from "/js/bm-randomart/bm-randomart-glsl.js"
...
function generateTree(k, depth) {
    if (depth >= k) return new (random.choice(terminals))();
...
export function generateArt(seed, tcanvas) {
    const numseed = CRC32(seed);
    let artformula = getArt(numseed);
    return renderGLSL(artformula, tcanvas);
}
