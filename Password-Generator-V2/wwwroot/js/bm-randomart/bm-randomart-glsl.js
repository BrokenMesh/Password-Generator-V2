...
export function renderGLSL(artformula, tcanvas, oversample = 2) {
    const ctx2d = tcanvas.getContext("2d");

    const renderW = tcanvas.width * oversample;
    const renderH = tcanvas.height * oversample;
    ...
    var v = shader_vert;
    var f = shader_frag.replace('$FORMULA', artformula.toString());
    ...
    if (!gl.getShaderParameter(vs, gl.COMPILE_STATUS)) {
        console.log(gl.getShaderInfoLog(vs), v);
        return;
    }

    if (!gl.getShaderParameter(fs, gl.COMPILE_STATUS)) {
        console.log(gl.getShaderInfoLog(fs), f);
        return;
    }
    ...
    function render_frame() {    
        gl.uniform1f(program.frameT, 0);
        gl.drawArrays(gl.TRIANGLES, 0, numItems);
        ctx2d.drawImage(gl.canvas, 0, 0, tcanvas.width, tcanvas.height);
    }
